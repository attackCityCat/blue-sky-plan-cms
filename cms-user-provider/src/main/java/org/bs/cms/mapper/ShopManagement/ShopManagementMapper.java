...
public interface ShopManagementMapper {
    @Update("UPDATE cms_product SET product_audit = 0 WHERE product_audit = 1")
    void editAdmShop();
    ...
    @Update("<script>"+
            "  	update   cms_product set product_audit = 2 where product_id in ( " +
            "		<foreach collection='array' index='index' item='item' separator=','> " +
            "			#{item} " +
            "		</foreach> " +
            ")</script>")
    void shenghe(Integer[] arr);
    ...
    @Select("<script>"+
            ...
            "left join cms_type stype on tpt.type_id = stype.id " +
            "left join cms_type btype on stype.pid = btype.id " +
            "left join cms_product_color tc on tpt.color_id = tc.id where  tpt.product_audit = 2 and tpt.product_id in ( " +
            "		<foreach collection='array' index='index' item='item' separator=','> " +
            "			#{item} " +
            "		</foreach> " +
            ")</script>")
    List<ProductBean> findProductListTwo(Integer[] arr);
    ...
    @Update("update cms_product set product_audit = 3 where product_id = #{value}")
    void editNoStates(Integer id);
}
